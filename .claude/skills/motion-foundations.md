...
| --------------------------------------- | --- | ------------------------------- |
| `import { motion } from "framer-motion"` | 1 | Use `motion/react` |
| `initial={{ opacity: 0 }}` on SSR component | 2 | Add mount guard |
| Skipping `useReducedMotion` check | 3 | Use `useSafeMotion` hook |
| `animate={{ width: "100%" }}` | 4 | Use `scaleX` transform instead |
| `transition={{ duration: 0.4 }}` inline | 5 | Use `motionTokens.duration.normal` |
| `{ stiffness: 300, damping: 30 }` inline | 6 | Use `springs.snappy` |
| Missing `"use client"` directive | 7 | Add to top of file |
| `navigator.hardwareConcurrency` at module level | 8 | Wrap in `typeof navigator !== "undefined"` |

## Related Skills

- **`motion-patterns`** — consumes tokens and springs defined here to build button, modal, stagger, page transition, and scroll patterns.
- **`motion-advanced`** — consumes tokens and springs defined here for drag, SVG, text, and gesture patterns.
