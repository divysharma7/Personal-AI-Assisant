...
import { Calendar, CheckSquare, Bell, BellRing } from 'lucide-react'
...
export default function AddItemModal({ open, onClose, onAdd, defaultDate, defaultStart, defaultEnd, defaultType, defaultTitle }: AddItemModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      if (defaultTitle) setTaskTitle(defaultTitle)
    }
  }, [open, defaultType, defaultStart, defaultEnd, defaultTitle])
      ...
      <div className="flex gap-1 mb-4 -mx-2">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            style={tab === id
              ? { background: 'var(--accent-dim)', color: 'var(--accent)', transition: 'background 0.15s cubic-bezier(0.16,1,0.3,1)' }
              : { color: 'var(--text-3)', transition: 'color 0.15s cubic-bezier(0.16,1,0.3,1)' }
            }
          >
            <Icon size={14} />
            {label}
