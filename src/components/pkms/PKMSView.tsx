...
export default function PKMSView() {
      ...
      if (isJournal) {
        if (activeNote) {
          ...
          const res = await fetch('/api/fs-notes', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: journalDate, parent: 'journal', content: json, date: journalDate }),
          })
  ...
  async function deleteFolder(id: string) {
    ...
    if (allIds.has(folderId)) setFolderId('root')
  }
  ...
  return (
    <div className="flex h-full w-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {isRoot && (
            <div className="px-2 pt-2">
              <p className="text-xs px-2 mb-1 font-medium tracking-wider" style={{ color: 'var(--text-3)' }}>
                FOLDERS
              </p>

              {/* Journal entry */}
              <button
                onClick={() => setFolderId('journal')}
                className="sidebar-item w-full"
              >
                <BookOpen size={13} className="flex-shrink-0" />
                <span className="truncate text-xs">Journal</span>
              </button>

              {folders.filter(f => f.parent === 'root').map(sf => (
          ...
          {/* ── Non-root: subfolders + back button ────────────────────── */}
          {!isRoot && !isJournal && (
            <div className="px-2 pt-2">
              <button
                onClick={() => {
                  const folder = folders.find(f => f._id === folderId)
                  setFolderId(folder?.parent ?? 'root')
                }}
                className="flex items-center gap-1 text-xs mb-2 px-2 py-1 hover:opacity-80 transition-opacity"
                style={{ color: 'var(--text-3)' }}
              >
                <ChevronLeft size={12} /> Back
              </button>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Editor body */}
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: 'var(--bg)', padding: '20px' }}>
          ...
          {!isJournal && (isRoot || !activeNote) && (
            <div className="flex flex-col items-center justify-center h-full gap-3"
              ...
              {isRoot
                ? <><Folder size={32} style={{ opacity: 0.3 }} /><p>Select or create a folder to begin</p></>
