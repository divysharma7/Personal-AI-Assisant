...
/**
 * PATCH /api/tasks/[id]/reparent
 *
 * Body: { parentId: string | null }
 * Directly sets the new parent. Updates depth and path accordingly.
 */
export async function PATCH(
  ...
  if (newParentId) {
    const newParent = await TaskModel.findById(newParentId).lean() as LeanDoc | null
    if (!newParent) {
      return NextResponse.json(
